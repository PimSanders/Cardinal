<template>
  <div>
    ...
    <el-button slot="reference" type="danger" style="margin-left: 10px;" @click="resetGameBox">{{ $t('gamebox.reset') }}</el-button>
    ...
    <el-table :data="gameBoxList" style="width: 100%" stripe v-loading="gameBoxList === null">
      ...
      <el-table-column :label="$t('general.operate')" width="300">
        <template slot-scope="scope">
          ...
          <el-button 
              size="mini" 
              @click="checkDown(scope.row.ID)">
            {{ $t('gamebox.check_down') }}
          </el-button>
          <el-button 
              size="mini" 
              type="danger" 
          ...
          <el-form :model="item" label-width="130px">
            ...
            <el-col :span="9">
              <el-form-item :label="$t('gamebox.ssh_password')">
                <el-input type="password" v-model="item.SSHPassword"></el-input>
              </el-form-item>
            </el-col>
            <el-col>
    ...
    <el-dialog :title="$t('gamebox.edit')" :visible.sync="editGameBoxDialogVisible">
      <el-form label-width="120px">
        ...
        <el-form-item :label="$t('gamebox.ssh_password')">
          <el-input type="password" v-model="editGameBoxForm.SSHPassword"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    checkDown(id) {
      this.utils.POST('/manager/checkDown', {GameBoxID: id}).then(res => {
        this.$message({message: res, type: 'success'})
        this.getGameBoxes()
      }).catch(err => this.$message.error(err))
    },
